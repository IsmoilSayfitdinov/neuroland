...
import { PaymentsTab } from "@/pages/admin/Children/components/PaymentsTab";
...
import { UchrashuvlarTab } from "@/pages/admin/Children/components/UchrashuvlarTab";
...
interface ChildDetailProps {
  child: ChildDetailOut | null | undefined;
  isLoading: boolean;
  isError?: boolean;
  onGoBack?: () => void;
  /** Extra action buttons rendered in the header (e.g. specialist buttons) */
  headerActions?: React.ReactNode;
}

export default function ChildDetail({ child, isLoading, isError, onGoBack, headerActions }: ChildDetailProps) {
  const [activeTab, setActiveTab] = useState<ChildTab>("basic");
  ...
  if (isError || !child) {
    return (
      <div className="py-20 text-center">
        ...
        {onGoBack && (
          <button onClick={onGoBack} className="mt-4 text-blue-500 font-bold hover:underline text-[13px]">
            Ro'yxatga qaytish
          </button>
        )}
      </div>
