...
export function TeamTab() {
  ...
  const openAdd = () => { reset({ name: "", title: "", bio: "", image_url: "", email: "", social_links: {} }); setIsAdding(true); setEditing(null); };
  const openEdit = (item: TeamMember) => { reset(item); setEditing(item); setIsAdding(false); };
  const closeForm = () => { setIsAdding(false); setEditing(null); };

  const onSubmit = (data: any) => {
    const payload = { ...data, social_links: data.social_links || {} };
    if (editing) {
      ...
      {(isAdding || editing) && (
        <form onSubmit={handleSubmit(onSubmit)} className="bg-[#F8F9FB] rounded-[16px] p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <input {...register("title")} placeholder="Lavozim" className="h-[44px] px-4 bg-white rounded-[10px] border border-gray-100 focus:border-[#4D89FF] outline-none text-[14px]" />
            <input {...register("email")} placeholder="Email" className="h-[44px] px-4 bg-white rounded-[10px] border border-gray-100 focus:border-[#4D89FF] outline-none text-[14px]" />
          ...
          <textarea {...register("bio")} placeholder="Biografiya" rows={2} className="w-full px-4 py-3 bg-white rounded-[10px] border border-gray-100 focus:border-[#4D89FF] outline-none text-[14px] resize-none" />
          ...
          <div key={member.id} className="p-4 bg-[#F8F9FB] rounded-[16px] group hover:bg-[#EEF4FF] transition-colors">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[12px] text-[#4D89FF]">{member.title}</p>
                <p className="text-[11px] text-[#9EB1D4] mt-1 line-clamp-2">{member.bio}</p>
              </div>
            </div>
